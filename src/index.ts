...
async function postToStaffLog(guildId: string, embed: EmbedBuilder, components: any[] = []) {
    ...
    } catch (e) { console.error("Staff log transmission failed"); }
}
...
const commands = [
  ...
  new SlashCommandBuilder().setName("remove-item").setDescription("Remove item from shop (Staff)").addStringOption(o => o.setName("item").setDescription("Item name").setAutocomplete(true).setRequired(true)),
...
// --- 4. Main Interaction Switch ---

client.on(Events.InteractionCreate, async (i: Interaction) => {
  try {
    // --- 4a. Autocomplete (Immediate response required) ---
    if (i.isAutocomplete()) {
    ...
    // --- 4b. Button Interactions ---
    if (i.isButton()) {
        // --- ⚡ CRITICAL FIX: DEFER BUTTONS IMMEDIATELY ⚡ ---
        ...
        if (i.customId.startsWith("gate_accept:") || i.customId.startsWith("gate_deny:")) {
            ...
            if (action === "gate_accept") {
                ...
                if (!p) return i.editReply("Error: Record missing.");
        ...
        if (i.customId === "btn_open_ticket") {
            ...
            await thread.send("**SOS Received.** <@" + i.user.id + ">, staff alerted.");
        ...
        if (reg && reg.status === 'verified') {
            ...
            if (i.customId === "claim_kit") {
                if (reg.hasClaimedKit) return i.editReply("❌ Kit already claimed.");
                ...
                return i.editReply("✅ Request sent to staff!");
            }
        ...
        // Handlers that require Modals (No reply needed here, just showModal)
        if (["btn_start_register", "btn_start_join", "btn_lft_start", "btn_alpha_claim", "add_task"].includes(i.customId)) {
            ...
            // Note: showModal cannot be preceded by a reply/defer. 
            // We use a different flow for modals.
            return i.showModal(m);
        }
    }

    // --- 4c. Chat Input Commands ---
    if (i.isChatInputCommand()) {
        // --- ⚡ CRITICAL FIX: DEFER COMMANDS IMMEDIATELY ⚡ ---
        await i.deferReply({ ephemeral: true }).catch(() => null);
        ...
        if (["post-info", "post-support", "post-alpha-terminal", "post-recruitment"].includes(i.commandName)) {
            ...
            return i.editReply({ content: "Terminal Deployed." });
        }
    }

    // --- 4d. Modal Submissions ---
    if (i.isModalSubmit()) {
        await i.deferReply({ ephemeral: true }).catch(() => null);
